...
with st.sidebar:
    ...
    if st.button("🌱 現在のチャットから分岐", use_container_width=True):
        branch_session()
    ...
    if current_messages:
        ...
        if st.button("🔗 外部共有リンクを作成", use_container_width=True):
            with st.spinner("リンクを生成中..."):
                try:
                    ...
                    with urllib.request.urlopen(req) as response:
                        ...
                        st.success("リンクを作成しました！")
                        st.code(share_url)
                        st.caption("※リンクは7日間有効です")
                except Exception as e:
                    st.error(f"リンク生成エラー: {e}")
    ...
    if paste:
        ...
        if pasted_image_bytes:
            st.success("画像が貼り付けられました！")
            st.image(pasted_image_bytes, caption="貼り付けた画像", use_container_width=True)
            if st.button("🗑️ 画像をクリア", key="clear_paste"):
    ...
    # ---- YouTube URL ----
    st.markdown("---")
    youtube_url = st.text_input("YouTube URL (字幕を分析)", placeholder="https://youtu.be/...")
    ...
    if mode_type == "選択1 (完全版)":
        ...
        st.caption("⚠️ ベータ版機能: 動作が不安定な場合があります")
    
    strict_mode = False  # 互換性維持のためのダミー変数
    st.markdown("---")
    
    st.title("設定")

    # ---- モデル選択（Gemini 1.5 は廃止されているので含めない） ----
    model_options = [
    ...
    if "3-pro" in model_id or "2.5-pro" in model_id:
        st.caption("🧠 高度な推論向け Pro モデル")
    if "flash" in model_id:
        st.caption("⚡ 高速・低コストな Flash 系モデル")
    ...
    # ---- 画像生成 ----
    st.subheader("🎨 画像生成")
    img_prompt = st.text_area("生成したい画像の説明", placeholder="未来的な都市の風景...")
    aspect_ratio = st.selectbox("アスペクト比", ["16:9", "1:1", "4:3", "3:4", "9:16"])

    generate_img_btn = st.button("画像を生成", type="primary", disabled=not img_prompt)
    ...
    if total_ratings > 0:
        ...
        for mod, stats in model_ratings.items():
            ...
            if rate > best_rate:
                ...
                best_model = mod
        if best_model != "N/A":
            st.caption(f"🏆 Best Model: **{best_model}** ({best_rate*100:.0f}%)")
    else:
        st.caption("まだ評価データがありません。")
    ...
    if usage_stats["total_cost_usd"] >= MAX_BUDGET_USD:
        st.error(f"⚠️ 予算上限 (${MAX_BUDGET_USD}) に達しました。")
